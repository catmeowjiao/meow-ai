...
def main():
    try:
        ...
        if config["base_url"] == None:
            print("".join([Fore.RED, "警告: 没有设置 Base URL, 开始配置"]))
            print()
            config["base_url"] = input(
                "".join([Fore.BLUE, "请输入您的 Base URL, 直接回车以使用 OpenAI 官方接口 > ", Style.RESET_ALL])
            )
            print()
        ...
        if config["api_key"] == None:
            ...
            config["api_key"] = input(
                "".join([Fore.BLUE, "请输入您的 API Key, 若没有请直接回车 > ",Style.RESET_ALL])
            )
            print()
            print("".join([Fore.GREEN,"设置完成!",Style.RESET_ALL]))
            print()
        if config["model"] == None:
            ...
            print()
            config["model"] = input(
                "".join([Fore.BLUE, "请输入要使用的模型, 直接回车以使用 gpt-3.5-turbo > ",Style.RESET_ALL])
            )
            print()
            print("".join([Fore.GREEN,"设置完成!",Style.RESET_ALL]))
            print()
